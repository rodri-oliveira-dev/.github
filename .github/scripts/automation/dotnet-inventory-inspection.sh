...
readonly INCLUDE_NON_PUBLIC_REPOSITORIES="false"

if [[ ! "$REPOSITORY_CLONE_TIMEOUT_SECONDS" =~ ^[1-9][0-9]*$ ]] ||
  (( REPOSITORY_CLONE_TIMEOUT_SECONDS > 600 )) ||
  [[ ! "$REPOSITORY_INSPECTION_TIMEOUT_SECONDS" =~ ^[1-9][0-9]*$ ]] ||
  (( REPOSITORY_INSPECTION_TIMEOUT_SECONDS > 3600 )); then
  echo "::error title=Invalid repository timeout::Clone timeout must be 1–600 seconds and inspection timeout 1–3600 seconds."
  exit 1
fi

if [[ ! "$INSPECTOR_VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ || ! "$INSPECTOR_SHA" =~ ^[0-9a-f]{40}$ ]]; then
  echo "::error title=Invalid Inspector provenance::Inspector version/SHA metadata is invalid."
  exit 1
fi
...
cleanup() {
  rm -f "$eligible_repositories_file" "$repositories_jsonl" "$projects_jsonl" "$problems_jsonl"
  rm -rf "$workspace_root"
}
trap cleanup EXIT

# Keep the GNU timeout diagnostic isolated from stdout/stderr emitted
# by untrusted repository code. Exit 124 or 137 alone does not prove
# that the deadline expired; the timeout-owned TERM diagnostic does.
run_bounded_command() {
...
cleanup_repository() {
  ...
  rm -f \
    ...
    "$inspector_stdout" \
    "$inspector_stderr" \
    "$timeout_diagnostics" || cleanup_failed=1
...
write_summary_markdown() {
...

  {
    ...
    echo
    printf 'Inspector: `%s` version `%s`, commit `%s`\n' "$INSPECTOR_REPOSITORY" "$INSPECTOR_VERSION" "$INSPECTOR_SHA"
    echo
    ...
    jq -r '
      ...
      | [
          ...
          ["Repository clone timeouts", .repository_clone_timeouts],
          ["Repository inspection timeouts", .repository_inspection_timeouts]
        ][]
      | "| \(.[0]) | \(.[1]) |"
    ' "$inventory_json"
...
jq -r '.[] | [.repository, .default_branch] | @tsv' \
  "$DISCOVERED_REPOSITORIES_FILE" \
  > "$eligible_repositories_file"
...
repository_clone_timeouts=0
repository_inspection_timeouts=0

echo ".NET repository inventory"
printf 'Inspector: v%s (%s)\n' "$INSPECTOR_VERSION" "$INSPECTOR_SHA"
...
while IFS=$'\t' read -r repository default_branch; do
  ...
  timeout_diagnostics="$(mktemp)"

  rm -rf "$repo_dir"

  # A timeout-owned diagnostic, not the clone exit code, proves
  # deadline expiry. The wrapper redirects clone stderr separately.
  if run_bounded_command "$REPOSITORY_CLONE_TIMEOUT_SECONDS" \
    "$inspector_stdout" "$inspector_stderr" "$timeout_diagnostics" \
    git clone \
      --depth 1 \
    ...
    clone_exit_code=$?
    if [[ "$BOUNDED_COMMAND_TIMED_OUT" == "true" ]]; then
      repository_clone_timeouts=$((repository_clone_timeouts + 1))
      record_problem "$repository" "clone_timeout" "Clone exceeded ${REPOSITORY_CLONE_TIMEOUT_SECONDS}s; process group terminated."
      inspection_warnings_errors=$((inspection_warnings_errors + 1))
      append_repository "$repository" "$visibility" "$default_branch" "clone_timeout" 0 "Clone timed out after ${REPOSITORY_CLONE_TIMEOUT_SECONDS}s."
      echo "::warning title=Repository clone timed out::$repository exceeded the ${REPOSITORY_CLONE_TIMEOUT_SECONDS}s clone limit."
      cleanup_repository
      print_repository_result "$repository_position" "$repositories_planned" "TIMEOUT" "$repository" "clone exceeded ${REPOSITORY_CLONE_TIMEOUT_SECONDS}s"
      continue
    fi
    record_problem "$repository" "clone" "Default branch could not be cloned (exit $clone_exit_code)."
    inspection_warnings_errors=$((inspection_warnings_errors + 1))
    append_repository "$repository" "$visibility" "$default_branch" "clone_failed" 0 "Default branch could not be cloned."
    echo "::warning title=Repository clone failed::Unable to clone $repository on $default_branch (exit $clone_exit_code)."
    cleanup_repository
  ...
  # Preserve the credential-free inspection boundary and terminate
  # dotnet/MSBuild descendants as one process group on timeout.
  if run_bounded_command "$REPOSITORY_INSPECTION_TIMEOUT_SECONDS" \
    "$inspector_stdout" "$inspector_stderr" "$timeout_diagnostics" \
    env \
      -u GH_TOKEN \
      -u GITHUB_TOKEN \
  ...
  if [[ "$BOUNDED_COMMAND_TIMED_OUT" == "true" ]]; then
    repository_inspection_timeouts=$((repository_inspection_timeouts + 1))
    record_problem "$repository" "inspection_timeout" "Inspector exceeded ${REPOSITORY_INSPECTION_TIMEOUT_SECONDS}s; dotnet/MSBuild process group terminated."
    inspection_warnings_errors=$((inspection_warnings_errors + 1))
    echo "::warning title=Repository inspection timed out::$repository exceeded the ${REPOSITORY_INSPECTION_TIMEOUT_SECONDS}s inspection limit."
    while IFS= read -r project_path; do
      [[ -z "$project_path" ]] && continue
      append_project_fallback "$repository" "$visibility" "$default_branch" "$project_path" "inspection_timeout"
    done < "$csproj_paths_file"
    append_repository "$repository" "$visibility" "$default_branch" "inspection_timeout" "$csproj_count" "Inspection timed out after ${REPOSITORY_INSPECTION_TIMEOUT_SECONDS}s."
    cleanup_repository
    print_repository_result "$repository_position" "$repositories_planned" "TIMEOUT" "$repository" "inspection exceeded ${REPOSITORY_INSPECTION_TIMEOUT_SECONDS}s"
    continue
  fi
...
jq -n \
  ...
  --arg inspector_version "$INSPECTOR_VERSION" \
  --arg inspector_sha "$INSPECTOR_SHA" \
  --arg inspector_ref "$INSPECTOR_SHA" \
  ...
  --argjson repository_clone_timeouts "$repository_clone_timeouts" \
  --argjson repository_inspection_timeouts "$repository_inspection_timeouts" \
  ...
  | {
      ...
      summary: {
        ...
        repository_clone_timeouts: $repository_clone_timeouts,
        repository_inspection_timeouts: $repository_inspection_timeouts
      },
...
printf 'Repository clone timeouts: %s\n' "$repository_clone_timeouts"
printf 'Repository inspection timeouts: %s\n' "$repository_inspection_timeouts"
