...
The `Validate governance source` job runs on every Pull Request and is the required governance merge gate for `main`. Pull Request path filters are intentionally not used for this workflow, ensuring the required check is always created even when a change does not touch agent-governance files.
